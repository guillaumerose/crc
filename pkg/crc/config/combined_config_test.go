...
func newTestConfig(configFile, envPrefix string) (*Config, error) {
	storage, err := NewCombinedStorage(configFile, envPrefix)
	if err != nil {
		return nil, err
	}
	config := New(storage)
...
func TestViperConfigBindFlagSet(t *testing.T) {
	...
	storage, err := NewCombinedStorage(configFile, "CRC")
	require.NoError(t, err)
	config := New(storage)
	config.AddSetting(CPUs, 4, ValidateCPUs, RequiresRestartMsg)
	config.AddSetting(NameServer, "", ValidateIPAddress, SuccessfullyApplied)
	...
	assert.Equal(t, SettingValue{
		Value:     5, // flag still has the priority
		IsDefault: false,
	}, config.Get(CPUs))
}
...
func TestTwoProcessesSharingSameConfiguration(t *testing.T) {
	dir, err := ioutil.TempDir("", "cfg")
	require.NoError(t, err)
	defer os.RemoveAll(dir)
	configFile := filepath.Join(dir, "crc.json")
...
func TestTwoProcessesWriteSameConfiguration(t *testing.T) {
	dir, err := ioutil.TempDir("", "cfg")
	require.NoError(t, err)
	defer os.RemoveAll(dir)
	configFile := filepath.Join(dir, "crc.json")
...
func TestTwoProcessesSetAndUnsetSameConfiguration(t *testing.T) {
	dir, err := ioutil.TempDir("", "cfg")
	require.NoError(t, err)
	defer os.RemoveAll(dir)
	configFile := filepath.Join(dir, "crc.json")
